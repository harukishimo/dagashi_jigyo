...
3. Issue種別を判定する。
4. 子Issueなら、要件定義、リサーチ、分析、作成、レビューの順でサブエージェントを起動する。
5. 主要Issueなら、レッドチーム、ブルーチーム、パープルチームを追加で起動する。
6. 統合用サマリ、エビデンス表、仮説、要検証、要調査の有無を確認する。
7. `Evidence Check` で主張分類と根拠を確認する。
8. `Review` でレビューエージェントの結果を確認する。
9. ジャッジエージェントで `Integration Ready` 可否を判定する。
10. 条件未達なら `Changes Requested` または `Blocked` にする。
11. 条件達成なら `Integration Ready` にする。
12. ナレッジ / ログ管理エージェントが履歴を記録する。
